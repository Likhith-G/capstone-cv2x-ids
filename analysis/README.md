...
| `baseline_starter.py` | **where an outside collaborator starts.** Loads the released bundle, builds the feature matrix, applies the frozen partition and runs either the published grouped cross-validation protocol or a single pass over the frozen split, then reports MCC, macro F1 and every class. Replace `build_model()` and change nothing else. It exits non-zero on a score high enough to be leakage, because every way of getting this protocol wrong flatters the result rather than hurting it, so the failure is silent otherwise |
...
| `make_package.sh` | wraps the release bundle into the two zips that actually get sent: 1.6 GB for all five scenarios and 358 MB for the reference one. Store mode, because every shard is already gzipped and deflating them again spends minutes to save nothing. The single scenario package carries all eight top level files beside the shards, because those are what `check_release.py` reads to know what it is looking at, and a scenario shipped without them verifies nothing |
