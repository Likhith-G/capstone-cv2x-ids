...
> **The Key Signal column is not a detection recipe.** `mean_pos_deviation`
> and `mean_speed_deviation` are computed by differencing a claimed value
> against the simulator's true position and speed. They identify the attack in
> this dataset, and a deployed roadside unit receives only the claim. Separately,
> no benign window here contains a single non-BSM packet, so `n_flood > 0`
> separates the five flood classes perfectly in this data and would not do so at
> a roadside unit that also carries ordinary non-V2X traffic.
